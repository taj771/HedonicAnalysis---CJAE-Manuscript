...
df_subset = df_subset %>%
  mutate(sale_month = month(date),
         waterfront = ifelse(wfront_l == 1 & dist_house_lake <= 100, 1, 0),
         ...
         dist_100m_250m = ifelse(between(dist_house_lake, 100,250),1,0),
         dist_250m_500m = ifelse(between(dist_house_lake, 250, 500),1,0),
         dist_500m_1000m = ifelse(between(dist_house_lake, 500, 1000),1,0),
         dist_1000m_2000m = ifelse(between(dist_house_lake, 1000,2000),1,0),
         log_sd_front = ifelse(waterfront == 1, log(sd_average), 0),
         log_sd_100_250 = ifelse(dist_100m_250m ==1, log(sd_average),0),
         ...
         log_sd_500_1000 = ifelse(dist_500m_1000m == 1, log(sd_average),0),
         log_sd_1000_2000 = ifelse(dist_1000m_2000m == 1, log(sd_average),0),
         ...
         log_sd_inter_100_250 = ifelse(dist_100m_250m == 1, log(sd_interp),0),
         ...
         log_sd_inter_1000_2000 = ifelse(dist_1000m_2000m == 1, log(sd_interp),0),
         ...
         inverse_dist = (1 / dist_house_lake),
         inverse_dist_500 = inverse_dist * dist_500m,
         ...
         inverse_dist = case_when(waterfront==1~1/100,
                                  waterfront==0~1/dist_house_lake),
         subset = paste0(closest_type, "-", data_type)) 

saveRDS(df_subset, "data/analysis_sample_wo_interpolationlimit.rds")
